import { AddOrUpdateHeaderAction, AllHeadersAction, RemoveHeaderAction } from "./header.actions";
import { addOrUpdate, pluckOut } from "../core";

export const removeHeaderReducer = (state, action) => {
    if (action instanceof RemoveHeaderAction)
        pluckOut({ items: state.headers, value: action.entity.id });
    return state;
}

export const addHeaderReducer = (state, action) => {
    if (action instanceof AddOrUpdateHeaderAction) {
...
export const allHeadersReducer = (state, action) => {
    if (action instanceof AllHeadersAction) {
